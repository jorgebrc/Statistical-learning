...
From this we can see that the worst Countries are the ones that are contributing more to the total Variance and that contributions are not equally distributed through the observations.
...
However, for `Population` it is 89%, which means that it can not be explained by a linear combination of these factors. We see that our model with 4 factors can explain 52% of the variance. Therefore, we will need to add more factors for better representing the model, which is not really ideal.
There are some variables that seem to be behave independently on the dataset like`Population`and `Measle`, as even using 6 factors their uniqueness is still high. So we are going to try again without these variables 
...
We are going to start with 3 clusters to mach our Categories for `Life Expectancy`
